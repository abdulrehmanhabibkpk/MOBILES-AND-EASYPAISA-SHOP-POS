...
import { QrCode, Printer, CheckCircle, Search, Trash2, FileText, Sliders, ShieldCheck, Tag, Layers, SlidersHorizontal, Sparkles } from 'lucide-react';
...
export const BarcodeStudioView: React.FC<BarcodeStudioViewProps> = ({ products, settings }) => {
  ...
  const renderBarcodeSvg = (code: string, customH?: number) => {
    ...
    const { totalWidth, rects } = generateBarcodeSvgElements(code || '1001', h, 2);
  ...
  return (
    <>
      {/* ========================================================================= */}
      {/* DIRECT BODY PORTAL: 100% RELIABLE PRINTING ON ALL BROWSERS (NO WHITE BLANK) */}
      {/* ========================================================================= */}
      {createPortal(
        <div id="barcode-print-portal" className="barcode-print-portal-root">
          {paperTemplate === '50x30' && (
            <div className="barcode-thermal-roll-50x30">
              ...
              {previewItems.map((item, idx) => (
                <div key={idx} className="barcode-sticker-50x30">
                  {showStoreHeader && (
                    <div style={{ fontSize: '7.5pt', fontWeight: '900', textTransform: 'uppercase', lineHeight: 1.1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%', color: '#000000' }}>
                      {shopTitle}
                    </div>
                  )}
                  {showProductTitle && (
                    <div style={{ fontSize: '7pt', fontWeight: '700', lineHeight: 1.1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%', marginTop: '0.5mm', color: '#000000' }}>
                      {item.title}
                    </div>
                  )}
                  <div style={{ width: '96%', margin: '0.5mm auto' }}>
                    {renderBarcodeSvg(item.code, 28)}
                    <div style={{ fontSize: '6.5pt', fontFamily: 'monospace', fontWeight: 'bold', letterSpacing: '1px', marginTop: '0.3mm', color: '#000000' }}>
                      {item.code}
                    </div>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '96%', borderTop: '0.5pt solid #000', paddingTop: '0.5mm', fontSize: '6.5pt', fontWeight: 'bold', color: '#000000' }}>
            ...
            <div className="barcode-thermal-roll-40x25">
              ...
              {previewItems.map((item, idx) => (
                <div key={idx} className="barcode-sticker-40x25">
                  {showStoreHeader && (
                    <div style={{ fontSize: '6.5pt', fontWeight: '900', textTransform: 'uppercase', lineHeight: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%', color: '#000000' }}>
                      {shopTitle}
                    </div>
                  )}
                  {showProductTitle && (
                    <div style={{ fontSize: '6pt', fontWeight: '700', lineHeight: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%', color: '#000000' }}>
                      {item.title}
                    </div>
                  )}
                  <div style={{ width: '96%', margin: '0.4mm auto' }}>
                    {renderBarcodeSvg(item.code, 22)}
                    <div style={{ fontSize: '5.5pt', fontFamily: 'monospace', fontWeight: 'bold', letterSpacing: '0.8px', color: '#000000' }}>
                      {item.code}
                    </div>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '96%', borderTop: '0.4pt solid #000', paddingTop: '0.3mm', fontSize: '5.5pt', fontWeight: 'bold', color: '#000000' }}>
                    {showBrand && <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '50%' }}>{item.brand}</span>}
              ...
              <div className="a4-grid-container">
                {previewItems.map((item, idx) => (
                  <div key={idx} className="barcode-sticker-a4">
                    {showStoreHeader && (
                      <div style={{ fontSize: '7.5pt', fontWeight: '900', textTransform: 'uppercase', lineHeight: 1.1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%', borderBottom: '0.3pt solid #ccc', paddingBottom: '0.3mm', marginBottom: '0.5mm', color: '#000000' }}>
                        {shopTitle}
                      </div>
                    )}
                    {showProductTitle && (
                      <div style={{ fontSize: '7.5pt', fontWeight: '700', lineHeight: 1.1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', width: '100%', color: '#000000' }}>
                        {item.title}
                      </div>
                    )}
                    <div style={{ width: '96%', margin: '0.8mm auto' }}>
                      {renderBarcodeSvg(item.code, 30)}
                      <div style={{ fontSize: '7pt', fontFamily: 'monospace', fontWeight: 'bold', letterSpacing: '1px', marginTop: '0.4mm', color: '#000000' }}>
                        {item.code}
                      </div>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '96%', borderTop: '0.4pt solid #000', paddingTop: '0.5mm', fontSize: '7pt', fontWeight: 'bold', color: '#000000' }}>
                      {showBrand && <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '50%' }}>{item.brand}</span>}
      ...
      {/* ========================================================================= */}
      <div className={`min-h-screen ${isLight ? 'bg-neutral-50 text-neutral-900' : 'bg-neutral-950 text-neutral-100'} p-3 sm:p-6 transition-colors print:hidden`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        ...
                        {filteredInventoryForSearch.length === 0 ? (
                          ...
                          filteredInventoryForSearch.map(p => (
                            <div
                              ...
                              className="p-2.5 text-xs hover:bg-emerald-50 cursor-pointer flex justify-between items-center transition-colors text-neutral-900"
                            >
                              <div>
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex items-center gap-1">
                              ...
                              <input
                                ...
                                className="w-16 py-1 px-2 rounded border text-xs font-bold text-center bg-white text-neutral-900"
                              />
                            </div>
                            <button
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className={`w-full py-2.5 px-3 rounded-xl border text-xs font-bold ${
                        isLight ? 'bg-white border-neutral-200 text-neutral-900' : 'bg-neutral-800 border-neutral-700 text-white'
                      }`}
                    >
                  ...
                  <div>
                    ...
                    <div className={`w-full py-2.5 px-3 rounded-xl border text-xs font-bold flex items-center justify-between ${
                      isLight ? 'bg-neutral-100 border-neutral-200 text-neutral-900' : 'bg-neutral-800 border-neutral-700 text-white'
                    }`}>
                      <span>Code 128 (Universal High-Density)</span>
                      <span className="text-[10px] text-emerald-600 bg-emerald-50 dark:bg-emerald-950/50 px-1.5 py-0.5 rounded font-bold">100% Scannable</span>
                    </div>
                  </div>
                </div>
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className={`p-5 rounded-2xl ${isLight ? 'bg-white border-neutral-200' : 'bg-neutral-900 border-neutral-800'} border shadow-sm sticky top-20`}>
                ...
                <div className="max-h-[520px] overflow-y-auto p-3 bg-neutral-100 dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 grid grid-cols-1 sm:grid-cols-2 gap-3">
